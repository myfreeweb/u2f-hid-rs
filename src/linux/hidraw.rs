...
#[repr(C)]
pub struct PlatformReportDescriptor {
    size: ::libc::c_int,
    value: [u8; 4096],
}
...
ioctl!(READ, hidiocgrdesc, b'H', 0x02; /*struct*/ PlatformReportDescriptor);

fn read_report_descriptor(fd: RawFd) -> io::Result<ReportDescriptor> {
    let mut desc = PlatformReportDescriptor {
        size: 0,
    ...
    let mut vec = Vec::from(&desc.value[..]);
    vec.truncate(desc.size as usize);
    Ok(ReportDescriptor {
        value: vec
    })
}
